...
import { ShoppingCart, User, Menu, X, Package, LayoutDashboard, Search, ChevronRight } from 'lucide-react';
...
export function Layout() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 font-sans">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            {isAuthenticated ? (
              <div className="hidden sm:flex items-center gap-2 border-l border-border pl-4 ml-2">
                <Link 
                  to="/admin" 
                  className="p-2.5 text-foreground/80 hover:text-primary hover:bg-secondary rounded-full transition-all"
                  title="Painel Admin"
                >
            ...
            <div className="border-t border-border pt-4">
              {isAuthenticated ? (
                <div className="flex flex-col gap-2">
                  <Link to="/admin" className="flex items-center justify-between py-3 px-4 rounded-lg hover:bg-secondary font-medium">
      ...
      {/* Main Content */}
      <main className={`flex-grow ${isAdmin ? 'pt-[72px]' : ''}`}>
        <Outlet />
      </main>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="border-t border-border pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <div className="flex gap-4">
                <Link to="/admin" className="text-xs text-muted-foreground hover:text-foreground font-medium transition-colors">
                  Área do Lojista
                </Link>
              </div>
            </div>
